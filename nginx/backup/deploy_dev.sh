#!/bin/bash
# E_Platform Development Nginx Deployment Script
# This script sets up Nginx as a reverse proxy for E_Platform in development mode
...
cp "$PROJECT_DIR/nginx/eplatform_dev.conf" "$NGINX_CONF_DIR/eplatform.conf"
...
if [ $? -eq 0 ]; then
    ...
    print_info "Your E_Platform should now be accessible at http://localhost"
...
print_info "1. Make sure your Django and Daphne services are running correctly"
print_info "2. Check the logs if you encounter any issues: $LOG_DIR/"
print_info "3. For production deployment, consider setting up HTTPS"
