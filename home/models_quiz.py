...
class Quiz(models.Model):
    ...
    def calculate_total_marks(self):
        ...
        # Since we don't have a marks field anymore, we'll set each question to be worth 1 point
        question_count = self.questions.count()
        # Only update passing_score if there are questions
        if question_count > 0:
            self.passing_score = max(int(question_count * 0.6), 1)  # 60% to pass, minimum 1
            self.save(update_fields=['passing_score'])
