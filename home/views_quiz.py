...
from .forms_quiz import QuizForm, QuestionForm, AnswerForm
...
def quiz_list(request):
    ...
    if request.user.is_staff or request.user.user_type == "instructor":
        ...
        quizzes = Quiz.objects.select_related("subject", "creator").all()
    else:
        # Students see only active quizzes that are in subjects they're enrolled in
        enrolled_subjects = [
            enrollment.subject.id
            for enrollment in request.user.enrollments.filter(is_active=True)
        ]

        quizzes = Quiz.objects.select_related("subject", "creator").filter(
            is_active=True, subject__id__in=enrolled_subjects
        )
...
def my_quizzes(request):
    ...
    try:
        if tab == "created" and (
            ...
            context = {"quizzes": quizzes, "tab": tab, "active_page": "quiz"}
            ...
            try:
                ...
                messages.error(
                    request, f"Unable to retrieve quiz attempts. Error: {str(e)}"
                )
                context = {"tab": tab, "active_page": "quiz", "error": str(e)}
                return render(request, "home/quiz/my_taken_quizzes.html", context)
    except Exception as e:
        # Catch any other errors
        messages.error(request, f"An error occurred: {str(e)}")
        return render(
            request, "home/error.html", {"error": str(e), "active_page": "quiz"}
        )
...
def quiz_detail(request, quiz_id):
    ...
    quiz = get_object_or_404(
        Quiz.objects.select_related("creator", "subject"), id=quiz_id
    )
    ...
    user_attempts = UserAttempt.objects.filter(user=request.user, quiz=quiz)

    context = {"quiz": quiz, "user_attempts": user_attempts, "active_page": "quiz"}
...
def create_quiz(request):
    ...
    context = {"form": form, "action": "create", "active_page": "quiz"}
...
def edit_quiz(request, quiz_id):
    ...
    context = {"form": form, "quiz": quiz, "action": "edit", "active_page": "quiz"}
...
def delete_quiz(request, quiz_id):
    ...
    context = {"quiz": quiz, "active_page": "quiz"}
...
def manage_questions(request, quiz_id):
    ...
    if not (request.user == quiz.creator or request.user.is_staff):
        messages.error(
            request, "You do not have permission to manage questions for this quiz."
        )
        return redirect("home:quiz_list")

    questions = quiz.questions.all().order_by("order")

    context = {"quiz": quiz, "questions": questions, "active_page": "quiz"}
...
def add_question(request, quiz_id):
    ...
    if not (request.user == quiz.creator or request.user.is_staff):
        messages.error(
            request, "You do not have permission to add questions to this quiz."
        )
        return redirect("home:quiz_list")
        ...
        if question_form.is_valid():
            ...
            if question.question_type == "true_false":
                Answer.objects.create(
                    question=question,
                    text="True",
                    is_correct=question_form.cleaned_data.get("correct_option")
                    == "true",
                    order=1,
                )
                Answer.objects.create(
                ...
                messages.success(
                    request, "Question added successfully with True/False options."
                )
...
def edit_question(request, question_id):
    ...
    if not (request.user == quiz.creator or request.user.is_staff):
        messages.error(
            request, "You do not have permission to edit questions in this quiz."
        )
        return redirect("home:quiz_list")
            ...
            if question.question_type == "true_false":
                try:
                    ...
                    true_answer.is_correct = (
                        question_form.cleaned_data.get("correct_option") == "true"
                    )
                    false_answer.is_correct = (
                        question_form.cleaned_data.get("correct_option") == "false"
                    )
...
def delete_question(request, question_id):
    ...
    if not (request.user == quiz.creator or request.user.is_staff):
        messages.error(
            request, "You do not have permission to delete questions from this quiz."
        )
        return redirect("home:quiz_list")
    ...
    context = {"quiz": quiz, "question": question, "active_page": "quiz"}
...
def manage_answers(request, question_id):
    ...
    if not (request.user == quiz.creator or request.user.is_staff):
        messages.error(
            request, "You do not have permission to manage answers for this quiz."
        )
        return redirect("home:quiz_list")
    ...
    if request.method == "POST":
        ...
        if formset.is_valid():
            ...
            if (
                ...
                and not question.answers.filter(is_correct=True).exists()
            ):
                messages.warning(
                    request, "Warning: No correct answer is marked for this question."
                )
...
def start_quiz(request, quiz_id):
    ...
    if existing_attempt:
        messages.info(
            request,
            "You already have an in-progress attempt. Continuing from where you left off.",
        )
    ...
    for question in questions:
        UserAnswer.objects.create(user_attempt=attempt, question=question)
    ...
    messages.success(
        request, f"You have {quiz.time_limit} minutes to complete this quiz. Good luck!"
    )
...
def take_quiz(request, attempt_id):
    ...
    try:
        ...
        if attempt.user != request.user:
            logger.warning(
                f"Unauthorized access: User {request.user.username} tried to access quiz attempt {attempt_id} belonging to {attempt.user.username}"
            )
            messages.error(
                request, "You do not have permission to access this quiz attempt."
            )
            return redirect("home:quiz_list")
        ...
        if attempt.status != "in_progress":
            logger.info(
                f"User {request.user.username} tried to access completed quiz attempt {attempt_id}"
            )
        ...
        if attempt.is_timed_out():
            logger.info(
                f"Quiz attempt {attempt_id} by user {request.user.username} has timed out"
            )
            attempt.time_out()
            messages.warning(
                request,
                "Your quiz time has expired. Your answers have been submitted automatically.",
            )
            return redirect("home:quiz_result", attempt_id=attempt.id)
    except Exception as e:
        logger.error(f"Error in take_quiz view: {str(e)}", exc_info=True)
        messages.error(
            request, "An error occurred while loading the quiz. Please try again."
        )
        return redirect("home:quiz_list")
    ...
    if request.method == "POST":
        logger.info(
            f"Processing quiz submission for attempt {attempt_id} by user {request.user.username}"
        )
        try:
            # Process answers
            for question in questions:
                ...
                if (
                    question.question_type == "mcq"
                    or question.question_type == "true_false"
                ):
                    ...
                    if answer_id:
                        try:
                            ...
                            logger.debug(
                                f"User {request.user.username} selected answer {selected_answer.id} for question {question.id}"
                            )
                            ...
                            logger.warning(
                                f"Invalid answer ID {answer_id} for question {question.id}"
                            )
                            pass
                    ...
                    if text_answer:
                        logger.debug(
                            f"User {request.user.username} provided text answer for question {question.id}"
                        )
                        # Create or update the user answer
                        user_answer, created = UserAnswer.objects.update_or_create(
                            user_attempt=attempt,
            ...
            if "submit_quiz" in request.POST:
                logger.info(
                    f"User {request.user.username} submitted quiz attempt {attempt_id}"
                )
                attempt.submit()
            ...
            logger.info(
                f"User {request.user.username} saved progress for quiz attempt {attempt_id}"
            )
    ...
    user_answers = {
        ...
        for answer in UserAnswer.objects.filter(user_attempt=attempt)
    }
...
def quiz_result(request, attempt_id):
    ...
    logger.info(
        f"User {request.user.username} viewing quiz result for attempt {attempt_id}"
    )
    ...
    try:
        ...
        if not (
            ...
            or quiz.creator == request.user
            or request.user.is_staff
        ):
            logger.warning(
                f"Unauthorized access: User {request.user.username} tried to view quiz results for attempt {attempt_id} belonging to {attempt.user.username}"
            )
            messages.error(
                request, "You do not have permission to view these quiz results."
            )
            return redirect("home:quiz_list")
        ...
        user_answers_query = UserAnswer.objects.filter(
            user_attempt=attempt
        ).select_related("selected_answer")

        # Create a dictionary for faster lookup
        user_answers = {answer.question_id: answer for answer in user_answers_query}
        ...
        logger.info(
            f"Quiz result for attempt {attempt_id}: score {attempt.score}/{quiz.total_marks} ({score_percent:.1f}%)"
        )
        ...
        messages.error(
            request,
            "An error occurred while loading the quiz results. Please try again.",
        )
        return redirect("home:quiz_list")
...
def quiz_analytics(request, quiz_id):
    ...
    try:
        ...
        if not (request.user == quiz.creator or request.user.is_staff):
            logger.warning(
                f"Unauthorized access: User {request.user.username} tried to view analytics for quiz {quiz_id} created by {quiz.creator.username}"
            )
            messages.error(
                request, "You do not have permission to view analytics for this quiz."
            )
            return redirect("home:quiz_list")

        # Get completed attempts with user data in a single query
        completed_attempts = UserAttempt.objects.filter(
            quiz=quiz, status__in=["completed", "timed_out"]
        ).select_related("user")
        ...
        stats = completed_attempts.aggregate(
            avg_score=Avg("score"), total_attempts=Count("id")
        )
        ...
        if questions and total_attempts > 0:
            ...
            question_stats_data = (
                UserAnswer.objects.filter(
                    user_attempt__in=completed_attempts, question_id__in=question_ids
                )
                .values("question_id")
                .annotate(
                    correct_count=Count(
                        "id",
                        filter=models.Q(marks_obtained=models.F("question__marks")),
                    ),
            ...
            question_stats_lookup = {
                item["question_id"]: item for item in question_stats_data
            }

            # Build the question_stats list
            question_stats = []
            for question in questions:
                stats_data = question_stats_lookup.get(
                    question.id, {"correct_count": 0, "total_count": 0}
                )
        ...
        logger.info(
            f"Quiz analytics for quiz {quiz_id}: {total_attempts} attempts, avg score: {average_score:.2f}/{quiz.total_marks} ({average_percentage:.1f}%)"
        )
        ...
        messages.error(
            request,
            "An error occurred while loading the quiz analytics. Please try again.",
        )
        return redirect("home:quiz_list")
...
def mark_short_answer(request, answer_id):
    ...
    try:
        ...
        return JsonResponse(
            {"status": "success", "new_score": user_answer.user_attempt.score}
        )
